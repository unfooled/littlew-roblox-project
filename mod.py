...
def close_browser_tab():
    if OS == "Darwin":
        ...
        # Windows - use PowerShell to focus Firefox and send Ctrl+W
        subprocess.run(
            ['powershell', '-Command',
...
#  TRAINING MODE
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def training_mode(img_urls: list[str], sid: str):
    print("\n  [TRAINING MODE] Label each avatar")
    print("  b = bad (flag)  |  g = good (clean)  |  s = skip  |  q = quit\n")

    for i, url in enumerate(img_urls):
        print(f"  Opening player {i+1} in browser...")
        open_url(url)

        while True:
            answer = input(f"  Player {i+1} -> [b]ad / [g]ood / [s]kip / [q]uit: ").strip().lower()
            close_browser_tab()

            if answer == "q":
                print("  Stopped training for this server.")
                return
            elif answer == "s":
                print("  Skipped.\n")
                break
                ...
                if answer == "b":
                    ...
                    print(f"  Saved as FLAGGED -> {path.name}")
                    send_webhook(content=f"[MANUAL FLAG] Server: {sid}")
                    send_webhook(embed={
                        "title": "Manually flagged avatar",
                        "description": "Labeled as inappropriate by moderator",
                        "image": {"url": url},
                        "color": 0xFF0000,
                    })
                else:
                    ...
                    path.write_bytes(img_data)
                    print(f"  Saved as CLEAN -> {path.name}")
                print()
                break
            else:
    ...
    print(f"\n  Training library: {flagged_count} flagged, {clean_count} clean")
    if flagged_count >= 3 and clean_count >= 3:
        print("  Enough examples collected. AI auto mode is now active.")
    else:
...
#  AUTO MODE
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def auto_mode(img_urls: list[str], sid: str):
    print(f"\n  [AUTO MODE] Checking {len(img_urls)} avatars...")
    flagged_players = []
...
flagged_count, clean_count = count_examples()
AI_READY = flagged_count >= 3 and clean_count >= 3
print(f"Training library: {flagged_count} flagged, {clean_count} clean")
if AI_READY:
    print("AI auto mode active.")
else:
    print("Training mode active. Need 3 flagged + 3 clean to unlock AI auto mode.")
...
while True:
    ...
    while True:
        ...
        for server in servers:
            ...
            if AI_READY:
                auto_mode(img_urls, sid)
            else:
                training_mode(img_urls, sid)
